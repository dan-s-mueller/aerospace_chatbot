...
from aerospace_chatbot.ui import SidebarManager, replace_source_tags, display_sources, handle_sidebar_state
...
# Set demo expert
os.environ["EXPERT"] = "Space Mechanisms"
...
if 'no_sources' not in st.session_state:
    st.session_state.no_sources = False
...
with chat_section:
                ...
                try:
                    ...
                    with st.status('Generating response...') as status:
                        ...
                        try:
                            ...
                            message_content = st.session_state.qa_model_obj.result['messages'][-1].content
                            ...
                            if "no source citations were found" in str(e):   # TODO To handle pydantic wrapper, can't use NoSourceCitationsFound
                                # When no citations are found, display response without sources
                                logger.info(f"No source citations found: {e}")
                                st.session_state.no_sources = True

                                # The *cause* is likely the original Pydantic ValidationError
                                # TODO this is messy, fix later
                                cause = e.__cause__
                                if isinstance(cause, ValueError):
                                    # Extract the first error, for example
                                    errors = cause.errors()
                                    if errors:
                                        error_msg = errors[0].get("msg", "")
                                        # Remove the leading "Value error, " if you added that prefix
                                        prefix = "Value error, "
                                        if error_msg.startswith(prefix):
                                            error_msg = error_msg[len(prefix):].strip()

                                message_placeholder.markdown(error_msg)    # TODO Return error which looks like response. Fix to return actual response
                                response_time = time.time() - t_start
    ...
    with sources_col:
        ...
        if st.session_state.messages and len(st.session_state.messages) > 0:
            if not st.session_state.no_sources: # Only show sources if sources found
